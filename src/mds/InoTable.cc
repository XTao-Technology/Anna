...
void InoTable::replay_alloc_id(inodeno_t id) 
{
  ...
  dout(1) << "replay_alloc_id " << id << " from " << projected_free << "/" << free << dendl;
  if (free.contains(id)) {
    free.erase(id);
    projected_free.erase(id);
  } else {
...
void InoTable::replay_alloc_ids(interval_set<inodeno_t>& ids) 
{
  ...
  dout(1) << "replay_alloc_ids " << ids << " before version" << version << "projected_free/free "
	  << projected_free << "/" << free<< dendl;
  ...
  projected_version = ++version;
  dout(1) << "replay_alloc_ids " << ids << " after, version "<< version << " projected_free/free "
	  << projected_free << "/" << free << dendl;
}
...
{
  dout(10) << "is_marked_free: ino = 0x" << std::hex << id << " free: " << free
	   << " projected_free: " << projected_free << dendl;
