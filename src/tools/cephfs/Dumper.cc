...
int Dumper::dump(const char *dump_file)
{
  ...
  if (fd >= 0) {
    ...
    snprintf(buf, HEADER_LEN, "Ceph mds%d journal dump\n start offset %llu (0x%llx)\n       length %llu (0x%llx)\n    ad_release_pos %llu (0x%llx)\n    write_pos %llu (0x%llx)\n    format %llu\n    trimmed_pos %llu (0x%llx)\n%c",
	    role.rank, 
	    (unsigned long long)start, (unsigned long long)start,
	    (unsigned long long)len, (unsigned long long)len,
	    (unsigned long long)journaler.last_committed.ad_release_pos, (unsigned long long)journaler.last_committed.ad_release_pos, 
...
int Dumper::undump(const char *dump_file)
{
  ...
  long long unsigned start, len, write_pos, format, trimmed_pos, ad_release_pos;
  sscanf(strstr(buf, "start offset"), "start offset %llu", &start);
  sscanf(strstr(buf, "length"), "length %llu", &len);
  sscanf(strstr(buf, "ad_release_pos"), "ad_release_pos %llu", &ad_release_pos);
  ...
  h.expire_pos = start;
  h.ad_release_pos = ad_release_pos;
