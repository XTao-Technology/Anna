...
void MDLog::append()
{
  ...
  //Peng TODO: not sure ad_release_pos was involved here
...
void MDLog::_submit_thread()
{
  ...
  while (!mds->is_daemon_stopping()) {
	...
	ls->offset = write_pos;
      ...
      dout(20) << "_submit_thread " << write_pos << "~" << bl.length()
	      << " : " << *le << dendl;
...
void MDLog::trim(int m)
{
  ...
  dout(10) << "first pengsegments last_seq " << p->first << " ls->end "
	   << p->second->end << dendl;
  //Peng debug
  //for (map<uint64_t, LogSegment*>::iterator p = segments.begin();
  //     p != segments.end();
  //     ++p)  {
  //  dout(10) << "pengsegments last_seq " << p->first << " ls->end "
  //	    << p->second->end << dendl;
  //}

  // Peng trim the segments that metahunter already held
  dout(10) << " pengtrim ls->end " << p->second->end << " ad_release_pos " <<
    journaler->get_ad_release_pos() << dendl;
...
void MDLog::set_metahunter(Metahunter *metahunter_)
{
  assert(NULL != metahunter_);
  metahunter = metahunter_;
}
...
void MDLog::_recovery_thread(MDSInternalContextBase *completion)
{
  ...
  if (jp.back) {
    ...
    Journaler back(jp.back, mds->mdsmap->get_metadata_pool(), CEPH_FS_ONDISK_MAGIC,
        mds->objecter, logger, l_mdl_jlat, &mds->timer, mds->finisher, metahunter);
  ...
  Journaler *front_journal = new Journaler(jp.front, mds->mdsmap->get_metadata_pool(),
      CEPH_FS_ONDISK_MAGIC, mds->objecter, logger, l_mdl_jlat, &mds->timer, mds->finisher, metahunter);
...
bool MDLog::allow_mh()
{
  return journaler->allow_mh();
}

bool MDLog::ad_journal_init()
{
  // metahunter read start pos should be the last ad_release_pos
  //uint64_t ad_pos = journaler->get_expire_pos();
  return journaler->init_ad_read_pos();
}


void MDLog::ad_release_journal_entry(uint64_t seq, uint64_t len)
{
  journaler->update_release_pos(seq, len);
}

//Peng for debug purpose
uint64_t MDLog::get_release_pos()
{
  return journaler->get_ad_release_pos();
}

void MDLog::dec_ad_read_pos(uint64_t len)
{
  journaler->dec_ad_read_pos(len);
}

void MDLog::jentry_queue_wait(MDRequestRef& mdr, Metahunter* metahunter_)
{
  dout(1) << " jentry_queue_wait mdr->jlen " << mdr->jlen << dendl;
  C_MDS_RetryJentryRequest* c_hold = new C_MDS_RetryJentryRequest(metahunter_, mdr);
  journaler->insert_jhold_waitinglist(c_hold);
  return;
}


void MDLog::ad_read_journal_entry(journal_entry_t &jentry,
				  MDRequestRef& mdr, Metahunter* metahunter)
{
  //uint64_t ad_pos = journaler->get_expire_pos();
  //journaler->init_ad_read_pos(ad_pos);
  dout(20) << "ad request for read log entry start, mdr->jlen is " <<
    mdr->jlen << dendl;

  uint64_t total_len = 0;
  uint64_t jlen = 0;
  C_SaferCond ad_readable_waiter;


hold_again:
  if (!journaler->is_ad_readable() &&
      (journaler->get_ad_read_pos() < journaler->get_write_pos()) &&
      !journaler->get_error()) {
    //ad_read_pos not reach the end of journal yet,
    //C_SaferCond ad_readable_waiter;
    // ad read buffer data is not enough for servering this read

    dout(1) << "ad_read_buffer is not enough for servering this read mdr->jlen " <<
      mdr->jlen << " will add total_len " << total_len << dendl;
    mdr->jlen += total_len;
    //MDSInternalContextBase* c_hold = new C_MDS_RetryJentryRequest(this, mdr);
    //journaler->insert_jhold_waitinglist(c_hold);
    jentry.op = op_done;
    return;
  }
  // the journal error case will be added latter
  
  if (!journaler->is_ad_readable() &&
      journaler->get_ad_read_pos() == journaler->get_write_pos()) {
    // not to reply to analysis daemon util there is new journal entry
    // becomes available
    //break;  // for the local test
    // TODO: find a way to solve this

    //jentry.op = op_done;
    //return; // this return is for test
    dout(1) << "mh reach to the end of journal mdr->jlen "  << mdr->jlen <<
      " will add total_len " << total_len << dendl;
    mdr->jlen += total_len;
    //MDSInternalContextBase* c_hold = new C_MDS_RetryJentryRequest(this, mdr);
    //journaler->insert_jhold_waitinglist(c_hold);
    jentry.op = op_done;
  ...
  // Peng it is still possible that journaler still not readable
  //assert(journaler->is_ad_readable());
  
  uint64_t pos = journaler->get_ad_read_pos();
  bufferlist bl;
  bool r = journaler->try_ad_read_entry(bl, jlen);
  if (!r && journaler->get_error()) {
    dout(1)<<"try_ad_read_entry error....." << dendl;
    // Peng TODO: 
    assert(0);  //fuck you .....
    //goto hold_again;
  }
  ...
  LogEvent *le = LogEvent::decode(bl);
  if (!le) {
    dout(0) << "_ad_replay " << pos << "~" << bl.length() << " / " <<
      journaler->get_write_pos() << " -- unable to decode event " << dendl;
  ...
  // please pay attention that the mds_lock was held at the very beginning of
  //the MDS::ms_dispatch
  //mds->mds_lock.Lock();
  if (EVENT_UPDATE != le->get_type()) {
    //journaler->add_to_release_pos(jlen);
    total_len += jlen;
    bl.clear();
    delete le;
    goto hold_again;
  ...
  le->ad_replay(mds, jentry);
  if (op_none == jentry.op) {
    //journaler->add_to_release_pos(jlen);
    total_len += jlen;
    bl.clear();
    delete le;
  ...
  total_len += jlen;
  uint64_t real_pos = journaler->get_ad_read_pos() - total_len;

  jentry.seq = real_pos - mdr->jlen;
  jentry.len = total_len + mdr->jlen;
  // move the seq managment to client side
  //journaler->jentry_buffer_insert(&jentry, jentry.seq);
...
void MDLog::ad_read_journal_entry_leak(journal_entry_t &jentry,
				       MDRequestRef& mdr, Metahunter* metahunter)
{
  //uint64_t ad_pos = journaler->get_expire_pos();
  //journaler->init_ad_read_pos(ad_pos);
  ...
  hold_again:
	...
	!journaler->get_error()) {
      ...
      dout(1) << "ad_read_buffer is not enough for servering this read mdr->jlen " <<
	mdr->jlen << " will add total_len " << total_len << dendl;
      //journaler->wait_for_ad_readable(&ad_readable_waiter);
      //ad_readable_waiter.wait();
      mdr->jlen += total_len;
      C_MDS_RetryJentryRequest* c_hold = new C_MDS_RetryJentryRequest(metahunter, mdr);
      journaler->insert_jhold_waitinglist(c_hold);
      //journaler->jhold_wl.push_back(c_hold);
	...
	" will add total_len " << total_len << dendl;
      ...
      // flush the write buffer to get the up to date entry, otherwise, the data left in
      // write buffer will not be awared of by the metahunter
      //journaler->ad_force_flush();
      jentry.op = op_done;
    ...
    //the MDS::ms_dispatch
    //mds->mds_lock.Lock();
    dout(1) << "LogEvent _type is " << le->get_type() << dendl;
    if (EVENT_UPDATE != le->get_type()) {
      //journaler->add_to_release_pos(jlen);
      total_len += jlen;
      bl.clear();
...
void MDLog::dispatch_request(MDRequestRef& mdr)
{
  if (mdr->killed) {
    dout(1) << "jentry hold request " << *mdr << " was killed" << dendl;
    return;
  }
  assert(NULL != mdr->client_request);
