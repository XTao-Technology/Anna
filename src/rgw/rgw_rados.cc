 ...
 * Placement scheduler:
 *
 * Randomly select one placement from the available options.
 * */
string RGWRados::random_sched(map<string, RGWZoneGroupPlacementTarget> placement_targets)
{
  int size = placement_targets.size();
  if (size > 0) {
    int index = rand() % size;

    map<string, RGWZoneGroupPlacementTarget>::iterator it = placement_targets.begin();
    for (int cur = 0; it != placement_targets.end(); it++, cur++) {
      if (cur == index) {
        return it->first;
      }
    }
...
string RGWRados::select_placement(list<string>& user_tags, map<string, RGWZoneGroupPlacementTarget> placement_targets)
{
  map<string, RGWZoneGroupPlacementTarget>::iterator it;
  // if user's tag list is empty, use the placements with '.default' tag
  std::list<std::string> tags (1, ".default");
  
  /* filter out unavailable placement targets */
  if (user_tags.size() != 0) {
    tags = user_tags;
  }
  
  std::list<string>& final_user_tags = tags;
  for (it = placement_targets.begin(); it != placement_targets.end(); it++) {
    struct RGWZoneGroupPlacementTarget& target = it->second;
    if (it->first == "default-placement" || !target.user_permitted(final_user_tags)) {
      placement_targets.erase(it->first);
    }
  }

  /* Choose random scheduler */
  return random_sched(placement_targets);
}
...

{
  ...
  if (rule.empty()) {
    /* select a rule based on user's tags */
    map<string, RGWZoneGroupPlacementTarget> placement_targets = zonegroup.placement_targets;
    rule = select_placement(user_info.placement_tags, placement_targets);
    if (rule.empty()) {
      ldout(cct, 0) << "can not find suitable placement" << dendl;
      return -EPERM;
    }
  } else {
