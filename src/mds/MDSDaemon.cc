...
int MDSDaemon::init()
{
  ...
  messenger->add_dispatcher_tail(&metahunter);
...
void MDSDaemon::handle_mds_map(MMDSMap *m)
{
  ...
  if (whoami == MDS_RANK_NONE) {
    ...
    if (mds_rank == NULL) {
      ...
      mds_rank->init();
      //Peng mh
      metahunter.init(mds_rank->mdlog, mds_rank);
    }
...
void MDSDaemon::suicide()
{
  ...
  // Peng hold the mh_lock is to prevent the risk with metahunter io path
