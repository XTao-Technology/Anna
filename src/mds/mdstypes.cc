...
//Peng metahunter
void ad_attr::encode(bufferlist& bl) const
{
  ::encode(mode, bl);
  ::encode(uid, bl);
  ::encode(gid, bl);
...
void ad_attr::decode(bufferlist::iterator &p)
{
  ::decode(mode, p);
  ::decode(uid, p);
  ::decode(gid, p);
...
journal_entry::journal_entry(int seq_num, ad_op_type_t op_type, inodeno_t par_ino,
			     inodeno_t ino, string dname, string par_dname,
			     ad_attr_t* attr, ad_attr_t* par_attr, map<string, bufferptr> xattr,
			     inodeno_t par_ino2, inodeno_t ino2, string dname2, string par_dname2,
			     ad_attr_t* attr2, ad_attr_t* par_attr2)
  : seq(seq_num), op(op_type), parentid(par_ino), fid(ino), name(dname),
    par_name(par_dname), pattr(NULL), par_pattr(NULL), xattr(xattr),
    parentid2(par_ino2), fid2(ino2), name2(dname2), par_name2(par_dname2),
    pattr2(NULL), par_pattr2(NULL)
{ 
  if(!attr) return;
  pattr = new ad_attr(*attr);
  if(!par_attr) return;
  par_pattr = new ad_attr(*par_attr);
  if (!attr2) return;
  pattr2 = new ad_attr(*attr2);
  if(!par_attr2) return;
  par_pattr2 = new ad_attr(*par_attr2);
}

journal_entry::journal_entry(const journal_entry& jentry)
{
  pattr = NULL;
  par_pattr = NULL;
  pattr2 = NULL;
  ...
  if (jentry.pattr)
    pattr = new ad_attr(*(jentry.pattr));
  if (jentry.par_pattr)
    par_pattr = new ad_attr(*(jentry.par_pattr)); 
  ...
  fid = jentry.fid;
  name = jentry.name;
  par_name = jentry.par_name;
  xattr = jentry.xattr;

  // for rename if it has
  if (jentry.pattr2)
    pattr2 = new ad_attr(*(jentry.pattr2));
  if (jentry.par_pattr2)
    par_pattr2 = new ad_attr(*(jentry.par_pattr2)); 
  
  parentid2 = jentry.parentid2;
  fid2 = jentry.fid2;
  name2 = jentry.name2;
  par_name2 = jentry.par_name2;
}

journal_entry& journal_entry::operator=(const journal_entry& jentry)
{
  if (this == &jentry)
    return *this;
  if (pattr2) {
...
void journal_entry::encode(bufferlist& bl) const
{
  //ENCODE_START(7, 5, bl);
  ::encode(seq, bl);
  ::encode(len, bl);
...
//::encode(attr, bl);
  ...
  if (op_rename == op) {
    ...
    //attr.encode(bl);
    pattr2->encode(bl);
    par_pattr2->encode(bl);
  }

  //ENCODE_FINISH(bl);
}

void journal_entry::decode(bufferlist::iterator &p)
{
  ...
  // it should be based on the op to decide whether decode
  // parent attr exists
  if (op_create == op ||
      op_mkdir == op ||
      op_unlink == op ||
      op_rmdir == op ||
      op_rename == op) {
    assert(NULL == par_pattr);
    par_pattr = new ad_attr();
    par_pattr->decode(p);
  }
  ::decode(xattr, p);
  ...
  if (op_rename == op) {
    ...
    pattr2 = new ad_attr();
    pattr2->decode(p);
    par_pattr2 = new ad_attr();
    par_pattr2->decode(p);
  }
  //DECODE_FINISH(bl);
}

void journal_entry::swap(journal_entry& jentry)
{
  std::swap(seq, jentry.seq);
  std::swap(len, jentry.len);
  std::swap(op, jentry.op);
  std::swap(parentid, jentry.parentid);
  std::swap(fid, jentry.fid);
  // swap the address of string, need the test
  std::swap(name, jentry.name);
  std::swap(par_name, jentry.par_name);
  std::swap(pattr, (jentry.pattr));
  if (op_create == op ||
      op_mkdir == op ||
      op_unlink == op ||
      op_rmdir == op ||
      op_rename == op) {
    assert (NULL != jentry.par_pattr);
    std::swap(par_pattr, (jentry.par_pattr));
  }
  xattr.swap(jentry.xattr);

  // for rename if it has
  if (op_rename == op) {
    std::swap(parentid2, jentry.parentid2);
    std::swap(fid2, jentry.fid2);
    // swap the address of string, need the test
    std::swap(name2, jentry.name2);
    std::swap(par_name2, jentry.par_name2);
    std::swap(pattr2, (jentry.pattr2));
    std::swap(par_pattr2, (jentry.par_pattr2));
  }
}
