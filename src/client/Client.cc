...
//yunfei clear the jentry_prefetch_buffer here
void Client::clear_jentry_prefetch_buffer()
{
  journal_buffer_lock.Lock();
  ldout(cct, 1) << "before clear, jentry_prefetch_buffer has "
		  <<jentry_prefetch_buffer.size()<< " entries."<<dendl;
  map< uint64_t, pair<__u8, mh_journal_entry*> >::iterator p =
    jentry_prefetch_buffer.begin();
  while (p != jentry_prefetch_buffer.end()) {
    // release the journal entry memory
    if (p->second.first ==  1) {
      delete p->second.second;
      p->second.second = NULL;
      jentry_prefetch_buffer.erase(p++);
    } else {
      p++;
    }
  }

  ldout(cct, 1) << "after clear, jentry_prefetch_buffer still has "
		  <<jentry_prefetch_buffer.size()<< " entries."<<dendl;

  if (!jentry_prefetch_buffer.empty()) {
    //yunfei set release_in_process
    release_in_process = jentry_prefetch_buffer.size();

    //traverse the map check if still hava a element's first is 1,
    //which means ours delete is not clean.
    //we also need to judge if the elements is continuous.
    p = jentry_prefetch_buffer.begin();
    //iterator q is begin's next
    map< uint64_t, pair<__u8, mh_journal_entry*> >::iterator q = ++p;
    p--;
    while (p != jentry_prefetch_buffer.end()) {
      // release the journal entry memory
      if (p->second.first ==  1) {
	ldout(cct, 1) << "clear_jentry_prefetch_buffer have a element's first is 1"
		      << dendl;
	assert(0);
      }
   
      if (p->second.second->seq + p->second.second->len != q->second.second->seq) {
	ldout(cct, 1) << "clear_jentry_prefetch_buffer not continuous" <<dendl;
	assert(0);
      }
      p++;
      q++;
      if (q == jentry_prefetch_buffer.end()) 
	break;
    }
  } else {
    //yunfei if jentry_prfetch_buffer is empty and the release_in_process is true
    // we must assert to look what had happen
    if (release_in_process)
      assert(0);
  }
  journal_buffer_lock.Unlock();
}
			       ...
			       std::string format, bufferlist& out)
{
  ...
  else if (command == "dump_journal_entry")
    m_client->dump_journal_entry(f);
...
Client::Client(Messenger *m, MonClient *mc)
  ...
  journal_buffer_lock("Client::journal_buffer_lock"),
  mds_name("")
{
  //yunfei add this variable
  prefetch_journal_outstanding = 0;
...
void Client::dump_journal_entry(Formatter *f)
{
  mh_journal_entry *pjentry = NULL;
  if (f)
    f->open_array_section("journal_entry");

  journal_buffer_lock.Lock();
  ldout(cct, 1) << "dump_journal_entry total size " << jentry_prefetch_buffer.size() << dendl;
  for (map< uint64_t, pair<__u8, mh_journal_entry*> >::iterator p = jentry_prefetch_buffer.begin();
       p != jentry_prefetch_buffer.end();
       ++p) {
    pjentry = p->second.second;
    ldout(cct, 1) << "dump jentry seq " << p->first << " processing state " << p->second.first <<
      " jentry op " << pjentry->op << " len " << pjentry->len << " fid " << pjentry->attr->fid.inode
		  << dendl;
  }
  journal_buffer_lock.Unlock();
...
int Client::init()
{
  ...
  ret = admin_socket->register_command("dump_journal_entry",
				       "dump_journal_entry",
				       &m_command_hook,
				       "show in-memory journal entry fetched from mds");
  if (ret < 0) {
    lderr(cct) << "error registering admin socket command: "
	       << cpp_strerror(-ret) << dendl;
  }
...
void Client::shutdown() 
{
  ...
  admin_socket->unregister_command("dump_journal_entry");
			 ...
			 bufferlist *pdirbl)
{
  ...
  //yunfei handle the case drop the release request if the mds server is not up
  if (request->get_abort_code() == JOURNAL_HOLD_RELEASE_ERROR) {
    if (request->head.op == CEPH_MDS_OP_JOURNAL_ENTRY_RELEASE){
      ldout(cct, 1) << "make_request received dropped journal release request, need to return 0"
	"seq: "<<request->head.args.mh.seq<< dendl;
      r = 0;
    } else {
      ldout(cct, 1) << "make_request received dropped journal hold request, need to return -1"
	"seq: "<<request->head.args.mh.seq<< dendl;
      r = -1;
    }
			  ...
			  bool drop_cap_releases)
{
  ...
  //yunfei set mdsmap_epoch
  request->head.mdsmap_epoch = mdsmap->get_epoch();
...
void Client::handle_mds_map(MMDSMap* m)
{
  ...
  for (map<mds_rank_t,MetaSession*>::iterator p = mds_sessions.begin();
    ...
    ldout (cct, 1) << "MetaSeesion:"<< p->second << "oldstate: "<< oldstate
		  << " newstate: " << newstate << dendl;

    //yunfei find that don't need to do clear here.The state is up and the inst is
    // different(inst's port and process_id-2 ),lead to must do the next clear_jentry_prefetch_buffer.
    /*if (oldstate != newstate && newstate == MDSMap::CEPH_MDS_STATE_REPLAY) {
      clear_jentry_prefetch_buffer();
      }*/
	...
	ldout (cct, 1) << "will trim cache for reconnect ... "<< p->second->inst << dendl;
	//yunfei clear the jentry_prefetch_buffer here
	clear_jentry_prefetch_buffer();
	trim_cache_for_reconnect(p->second);
      }
    } else if (oldstate == newstate) {
      ldout(cct, 1) << "handle_mds_map no change ... " << dendl;
      continue;  // no change
    }

    if (newstate == MDSMap::STATE_RECONNECT &&
	mds_sessions.count(p->first)) {
      ldout(cct, 1) << "handle_mds_map comming STATE_RECONNECT " << dendl;
	...
	// kick new requests
	ldout(cct, 1) << "handle_mds_map comming STATE_ACTIVE " << dendl;
...
void Client::kick_requests(MetaSession *session)
{
  ...
  for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin();
       p != mds_requests.end();
       ++p) {
    //yunfei drop all jhold and jrelease reuqests
    // becase we barrier all new hold request so jhold only have one case:
    //a,sent before mds down, which won't back forever
    //jrelease similar to jhold, all new release requests will be barriered and
    //the sent before mds down won't back either.what's more mds will read
    //head again when it start, so drop it decidedly.
    if (p->second->mds_name != "") {
      if (p->second->head.op == CEPH_MDS_OP_JOURNAL_ENTRY_RELEASE) {
	ldout(cct, 1) << "kick mds_requests jrelease drop it and signal it's caller: "
		      << p->second->head.args.mh.seq<< dendl;
	do_reply = true;
      }
      else if (p->second->head.op == CEPH_MDS_OP_JOURNAL_ENTRYLIST_HOLD) {
	ldout(cct, 1) << "kick mds_requests jhold drop it and signal it's caller: " 
		      << p->second->head.args.mh.seq<< dendl;
	do_reply = true;
      }
      if (do_reply) {
	MClientReply *reply = new MClientReply;

	//yunfei hardcode abort_rc = 1for drop this request
	p->second->abort(JOURNAL_HOLD_RELEASE_ERROR);
	p->second->reply = reply;
	p->second->caller_cond->Signal();
	unregister_request(p->second);
      }
      continue;
    }
  out:
    ...
    if (p->second->mds == session->mds_num) {
      //yunfei
      ldout(cct, 1) << "kick_requests send a request ..." << dendl;
...
void Client::resend_unsafe_requests(MetaSession *session)
{
  ...
  for (xlist<MetaRequest*>::iterator iter = session->unsafe_requests.begin();
       ...
       ++iter) {
    //yunfei
    ldout(cct, 1) << "unsafe_requests resend unsafe_requests  ..." << dendl;
  ...
  for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin();
    ...
    //head again when it start, so drop it decidedly.
    if (req->mds_name != "") {
      if (req->head.op == CEPH_MDS_OP_JOURNAL_ENTRY_RELEASE) {
	ldout(cct, 1) << "resend mds_requests jrelease drop it and signal it's caller: "
		      << req->head.args.mh.seq<< dendl;
	do_reply = true;
      }  else if (req ->head.op == CEPH_MDS_OP_JOURNAL_ENTRYLIST_HOLD) {
	ldout(cct, 1) << "resend mds_requests jhold  drop it and signal it's caller: "
		      << req->head.args.mh.seq<< dendl;
	do_reply = true;
      }
      if (do_reply) {
	...
	//yunfei hardcode abort_rc = 1for drop this request
	req->abort(JOURNAL_HOLD_RELEASE_ERROR);
	req->reply = reply;
  ...
  out:
    ...
    if (req->mds == session->mds_num) {
      ldout(cct, 1) << "unsafe_requests send mds_requests ..." << dendl;
...
void Client::wait_on_journal_buffer(list<Cond*>&ls)
{
  Cond cond;
  ls.push_back(&cond);
  cond.Wait(journal_buffer_lock);
  ls.remove(&cond);
}
...
//Peng
int Client::mh_open_journal(const char *mds)
{
  MetaRequest *req = NULL;
  // authenticate and start tick (heartbeat) here
  Mutex::Locker lock(client_lock);
  int r = authenticate();
  if (r < 0) {
    return r;
  }
  // the outstanding IO were kept in track in the tick
  //tick();

  mds_name = mds;
  while (1) {
    if(!mdsmap->is_up(mds_name) || !mdsmap->is_active(0)) {
      ldout(cct, 1) << " can't open .target mds." << mds_name <<
        " not active  waiting for new mdsmap" <<  dendl;
      wait_on_list(waiting_for_mdsmap);
      continue;
    }
    ldout(cct, 1)<< mds <<" is up and active, will connect to it."<< dendl;
    break;
  }
  req = new MetaRequest(CEPH_MDS_OP_JOURNAL_INIT);
  int res = make_request(req, 0, 0, 0, 0, 0, 0);
  if (res < 0) {
    ldout(cct, 1) << " mh_open_journal error with res " <<
      res << dendl;
  }
  return res;
}

int Client::ll_hold_journal()
{
  MetaRequest *req = NULL;
  Mutex::Locker lock(client_lock);
  
  // test not enable tick yet
  //tick();
  
  req = new MetaRequest(CEPH_MDS_OP_JOURNAL_ENTRY_HOLD);
  // choose mds 0 for test
  int res = make_request(req, 0, 0, 0, 0, 0, 0);
  ldout(cct, 1)<<"ad journal hold result = " << res << dendl;
  
  return res;
}

int Client::mh_release_journal(uint64_t seq)
{
  MetaRequest *req = NULL;
  //bool found = 0;
  uint64_t total_len = 0;
  uint64_t next_pos = seq;
  bool do_signal = false;
  Mutex::Locker lock(client_lock);

  ldout(cct, 20) << "_release_journal seq " << seq << dendl;
  if (seq == 0)
    return -EINVAL;

  journal_buffer_lock.Lock();
  // typical grammer error
  //assert((1 == jentry_prefetch_buffer[seq].first) &&
  // (NULL != jentry_prefetch_buffer[seq].second));

  if (!jentry_prefetch_buffer.count(seq)) {
    //assert(0) if the jentry doesn't exit in buffer we return directly,
    // because we may have cleared jentry_prefetch_buffer
    ldout(cct, 1)<< "mh_release_journal return directly, becasue the jentry "
		 "doesn't exit in jentry_prefetch_bufer"<<dendl;
    journal_buffer_lock.Unlock();
    return 0;
  }

  //Client inserts a jentry which the metahunter will release into jentry_prfetch_buffer
  //when metahunter reconnect to mds and this cause the first become 1 but it should
  //be 2. So we return directly and the jentry will be hold later on.
  if (1 == jentry_prefetch_buffer[seq].first) {
    ldout(cct, 1)<< "mh_release_journal return directly, becasue the jentry "
		 "firt is 1"<<dendl;
    journal_buffer_lock.Unlock();
    return 0;
  }
  assert(2 == jentry_prefetch_buffer[seq].first);
  assert(NULL != jentry_prefetch_buffer[seq].second);
  // clear the outstanding flag, 0 means mh finished the processing
  jentry_prefetch_buffer[seq].first = 0;
  while (!jentry_prefetch_buffer.empty()) {
    map< uint64_t, pair<__u8, mh_journal_entry*> >::iterator p =
      jentry_prefetch_buffer.begin();
    if (p->first != next_pos) {
      ldout(cct, 20) << " mh_release_journal not continous, next_pos " <<
	next_pos << " p->first " << p->first << dendl;
      break;
    }
    
    if (p->second.first) {
      // can not align this one which is  in the metahunter processing
      ldout(cct, 20) << " mh_release_journal skip in-progress jentry seq " << 
		    p->first << dendl;
      break;
    }
    next_pos = p->second.second->seq + p->second.second->len;
    total_len += p->second.second->len;
    // release the journal entry memory
    delete p->second.second;
    p->second.second = NULL;
    jentry_prefetch_buffer.erase(p);
  ...
  if (release_in_process || do_signal) {
    if (do_signal) {
      assert(jentry_prefetch_buffer.empty());
      ldout(cct, 1) << "mh_release_journal signal waiting_for_buffer_clean"<<dendl;
      signal_cond_list(waiting_for_buffer_clean);
    }
    journal_buffer_lock.Unlock();
    ldout(cct, 1) << "mh_release_journal drop decidedly by reason of release_in_process "
		  << seq <<dendl;
    return 0;
  }
  journal_buffer_lock.Unlock();
  ...
  if (0 == total_len) {
    // means not continous, let next round to align this
    ldout(cct, 20) << " mh_release_journal not found continue release pos " <<
      seq << dendl;
    return 0;
  }
  //yunfei
  if (!mdsmap->is_up(mds_name) || !mdsmap->is_active(0)) {
    ldout(cct, 1) << "mh_release_journal target mds "
		  << mds_name <<" is not active, so drope it"
		  <<  dendl;
    return 0;
  }

  // where to release the new allocated req memory
  req = new MetaRequest(CEPH_MDS_OP_JOURNAL_ENTRY_RELEASE);
  req->mds_name = mds_name;
  req->head.args.mh.seq = seq;
  req->head.args.mh.len = total_len;

  int res = make_request(req, 0, 0, 0, 0, 0);
  if (res < 0) {
    ldout(cct, 1) << " mh_release_journal make request error with res " <<
      res << dendl;
  }
  return res;
}

int Client::mh_get_root(void *buffer, int buflen)
{
  mh_attr *rootattr = new mh_attr();
  rootattr->fid.inode = root->ino.val;
  rootattr->mode = root->mode;
  rootattr->nlink = root->nlink;
  rootattr->uid = root->uid;
  rootattr->gid = root->gid;
  rootattr->size = root->size;

  rootattr->atime = root->atime;
  rootattr->ctime = root->ctime;
  rootattr->mtime = root->mtime;
  
  memcpy(buffer, rootattr, buflen);
  delete rootattr;
  return 0;
}

void Client::retry_prefetch_journal()
{
  Mutex::Locker lock(journal_buffer_lock);
  if (!prefetch_journal_outstanding) {
    async_prefetch_journal.queue(new C_Client_PrefetchJentryList(this));
    prefetch_journal_outstanding++;
  }
}

int Client::mh_hold_journal(void *buffer, int buflen)
{
  mh_journal_entry *pjentry = NULL;
  bool found = 0;
  Mutex::Locker lock(journal_buffer_lock);
...
retry_hold:
  // first read from the prefetch buffer
  for (map<uint64_t, pair<__u8, mh_journal_entry*> >::iterator p =
	 jentry_prefetch_buffer.begin(); p != jentry_prefetch_buffer.end();
       ++p) {
    if (1 == p->second.first) {
      found = 1;
      // 1 means in just prefetched back not in processing yet
      p->second.first = 2;
      pjentry = p->second.second; 
      // FIXME: this looks ugly
      // as xattrs is not fixed size, we must copy more 
      
      memcpy(buffer, pjentry, buflen); 
      break;
    } else {
      // 0 or 2 means either in "finished processing or in processing", where
      // we will skip it
      continue;
    }
  }
  ...
  if (!found) {
    ldout(cct, 10) << " jentry_prefetch_buffer is empty need prefetch and wait "<<dendl;
    if (!prefetch_journal_outstanding) {
      async_prefetch_journal.queue(new C_Client_PrefetchJentryList(this));
      prefetch_journal_outstanding++;
    }
    wait_on_journal_buffer(waiting_for_prefetch_journal);
    goto retry_hold;
  }

  if (jentry_prefetch_buffer.size() > 500) {
    return 0;
  }

  if (!prefetch_journal_outstanding) {
    // async prefetch, please note that it was protected by teh journal buffer lock
    async_prefetch_journal.queue(new C_Client_PrefetchJentryList(this));
    prefetch_journal_outstanding++;
  }
  return 0;
...
/* Encode C++ Map<String, bufferptr> in xt_map format */
const uint16_t xtao_map_magic_num = (uint16_t)0x1234;
const uint16_t xtao_map_version = (uint16_t)0x1; 
xt_map_t* Client::xt_map_encode(std::map<std::string, bufferptr>& map)                                     
{                                                                                                   
    // calculate buffer size                                                                        
    uint32_t kv_size = 0;                                                                           
    uint32_t num = 0;                                                                               
    std::map<std::string, bufferptr>::iterator p = map.begin();                                   
    for (; p != map.end(); ++p) {                                                                   
        ldout(cct, 20) << __FUNCTION__ << " key: " << p->first << " val: " << p->second << dendl; 
        kv_size += p->first.length() + p->second.length() + 2; // plus tail '\0'                                          
        num ++;                                                                                     
    }                                                                                               
                                                                                                    
    uint32_t total_size = kv_size + sizeof(xt_map_t) + sizeof(xtao_map_magic_num) + num * sizeof(xt_me);
    ldout(cct, 20) << __FUNCTION__ << ": num " << num << ", kv_size: " << kv_size << ", total_size: " << total_size \
        << ", sizeof(xt_map_t): " << sizeof(xt_map_t) << ", sizeof(xtao_map_magic_num): " << sizeof(xtao_map_magic_num) << dendl;
        
    // overflow? 
    if (total_size <= kv_size) {
	lderr(cct) << __FUNCTION__ << "uint32_t total_size overflow: total_size(" << total_size << "), kv_size(" << kv_size << ")" << dendl;
        return NULL;
    }
                       
    uint8_t* base = (uint8_t*)malloc(total_size);                                                                     
    if (!base) {                                                                                    
        lderr(cct) << __FUNCTION__ << ": allocate memory for xt_map_t failed" << dendl;                         
        return NULL;                                                                                
    } else {                                                                                        
        ldout(cct, 20) << __FUNCTION__ << ": base " << (void*)base << dendl;                         
    }                                                                                               
                                                                                                    
    // fill in xt_map                                                                               
    xt_map_t* xt_map = (xt_map_t*)base;                                                             
    xt_map->num = num;                                                                              
    xt_map->version = xtao_map_version;                                                             
    xt_map->total_size = total_size;                                                                
    p = map.begin();                                                                                
    uint32_t offset = sizeof(xt_map_t);                                      
    for (; p != map.end(); ++p) {                                                                   
        ldout(cct, 20) << __FUNCTION__ << ": offset " << offset << dendl;                            
        xt_me* me = (xt_me*)(base + offset);                                                        
        me->kl = p->first.length();                                                                 
        me->vl = p->second.length();                                                                
                                                                                                    
        // copy key                                                                                 
        char* key = (char*)((char*)me + sizeof(xt_me));                                             
        p->first.copy(key, me->kl);                                                                 
                                                                                                    
        // copy value                                                                               
        char* val = (char*)(key + me->kl);    
        *val++ = '\0';
        p->second.copy_out((unsigned)0, (unsigned)(me->vl), val);
        *(val + me->vl) = '\0';                                                                
                                                                                                    
        offset += me->kl + me->vl + sizeof(xt_me) + 2;                                                  
    }                                                                                               
                                                                                                    
    // set magic number                                                                             
    *(uint16_t*)((uint8_t*)base + offset) = xtao_map_magic_num;                                     
                                                                                                    
    return xt_map;                                                                                  
}                               
 
int Client::prefetch_journallist()
{
  MetaRequest *req = NULL;
  bufferlist mh_bl;
  int len = 0;
  ...
  // test not enable tick yet
  //tick();
  // ivan waiting until the jentry_prefetch_buffer is clean
  // if release_in_process is true
  if (release_in_process) {
    ldout(cct, 1) << "jentry_prefetch_buffer is not clean, so wait it." << dendl;
    wait_on_list(waiting_for_buffer_clean);
  }

  //ivan waiting the mds_map changed
  while (!mdsmap->is_up(mds_name) || !mdsmap->is_active(0)) {
    ldout(cct, 1) << "target mds  " << mds_name <<
      " is not active  or not up waiting for new mdsmap, now epoch: " <<
      mdsmap->get_epoch() <<  dendl;
      wait_on_list(waiting_for_mdsmap);
      continue;
  }
  ldout(cct, 20) << "prefetch_journallist new a request meta_mds: " <<
    mds_name<< dendl;

  req = new MetaRequest(CEPH_MDS_OP_JOURNAL_ENTRYLIST_HOLD);
  req->mds_name = mds_name;

  // choose mds 0 for test
  int res = make_request(req, 0, 0, 0, 0, 0, &mh_bl);
  if (res < 0) {
    ldout(cct, 1) << "prefetch_journallist make request error with res " <<
      res << dendl;
    //yunfei
    journal_buffer_lock.Lock();
    assert(prefetch_journal_outstanding == 1);
    prefetch_journal_outstanding--;
    signal_cond_list(waiting_for_prefetch_journal);
    journal_buffer_lock.Unlock();
    return res;
  }

  // decode bl to journal entry
  // TODO: we need find a way to recycle the new journal entry
  //*ppjentry = new mh_journal_entry();
  //struct journal_entry reply_jentry;
  bufferlist::iterator p = mh_bl.begin();
  ::decode(num_jentry, p);
  assert(num_jentry > 0);

  //for this case:
  //a, replay come back
  //b, mds_down and handle_mds_map hold client lock and change mdsmap state
  //c, prefetch_journallist hold lock come here
  if (!mdsmap->is_active(0) || release_in_process != 0) {
    mh_bl.clear();
    ldout(cct, 1) << "the buffer is deleteting, so drop the mh_bl and "
      "signal the waiters of waiting_for_prefetch_buffer"<<dendl;
    goto out;
  }

  for (uint32_t i = 0; i < num_jentry; i++) {
    struct journal_entry reply_jentry;
    reply_jentry.decode(p);

    struct mh_journal_entry *pjentry = new mh_journal_entry();
    pjentry->seq = reply_jentry.seq;
    pjentry->len = reply_jentry.len;
    pjentry->op = (mh_op_type_t)(reply_jentry.op);
    assert(NULL == pjentry->pattr);
    assert(NULL == pjentry->attr);

    if ((mh_op_create ==  pjentry->op) ||
	(mh_op_mkdir == pjentry->op) ||
	(mh_op_unlink == pjentry->op) ||
      (mh_op_rmdir == pjentry->op) ||
      (mh_op_rename == pjentry->op)) {
      // we need the parent attr
      pjentry->pattr = new mh_attr();
    }
    if (mh_op_rename == pjentry->op) {
      pjentry->pattr2 = new mh_attr();
    }
    if ((mh_op_rename == pjentry->op) &&
	(0 != reply_jentry.fid2.val)) {
      pjentry->attr2 = new mh_attr();
      pjentry->attr2->fid.inode = reply_jentry.fid2;
    }

    pjentry->xattr = NULL; // xattr is invalid
#if 0
/// uncomment this to enable xattr log replay
    if (mh_op_setattr == pjentry->op) { 
      // memery is allocated in xt_map_encode
      pjentry->xattr = (void*)xt_map_encode(reply_jentry.xattr);
      if (!pjentry->xattr) {
        lderr(cct) << "xt_map_encode() failed" << dendl; 
	return -ENOMEM;	
      }
    }
#endif
    pjentry->attr = new mh_attr();
    // Peng debug for mem leak
    //assert(NULL != reply_jentry.pattr);
    pjentry->attr->parentid.inode = reply_jentry.parentid;
    pjentry->attr->fid.inode = reply_jentry.fid;
    len = reply_jentry.name.length();
    pjentry->name = new char[len + 1];
    //jentry.name = (char*)(reply_jentry.name.data());
    //strcpy(pjentry->name, (char*)(reply_jentry.name.data()));
    reply_jentry.name.copy(pjentry->name, len, 0);
    pjentry->name[len] = '\0';
    
    if (reply_jentry.pattr) { 
      pjentry->attr->fid.validator = reply_jentry.pattr->ctime;
      pjentry->attr->mode = reply_jentry.pattr->mode;
      pjentry->attr->uid = reply_jentry.pattr->uid;
      pjentry->attr->gid = reply_jentry.pattr->gid;
      pjentry->attr->nlink = reply_jentry.pattr->nlink;
      pjentry->attr->ctime = reply_jentry.pattr->ctime;
      pjentry->attr->atime = reply_jentry.pattr->atime;
      pjentry->attr->mtime = reply_jentry.pattr->mtime;
      // if is dir, size equals rstat->rbytes
      // if is file, size equals inode->size
      pjentry->attr->size = reply_jentry.pattr->size;
      if (reply_jentry.pattr->is_dir()) {
	pjentry->attr->blocks = 1;
      } else {
	pjentry->attr->blocks = (reply_jentry.pattr->size + 511) >> 9;
      }
      pjentry->attr->blksize = MAX(root->layout.stripe_unit, 4096);
    }
    
    if (reply_jentry.par_pattr) {
      pjentry->pattr->parentid.inode = 0;
      pjentry->pattr->fid.inode = reply_jentry.parentid;
      pjentry->pattr->fid.validator = reply_jentry.par_pattr->ctime;
      pjentry->attr->parentid.validator = reply_jentry.par_pattr->ctime;
      len = reply_jentry.par_name.length();
      // name2 was for rename, without rename it used here for debug
      //pjentry->name2 = new char[len+1];
      //reply_jentry.par_name.copy(pjentry->name2, len, 0);
      //pjentry->name2[len] = '\0';
      
      pjentry->pattr->mode = reply_jentry.par_pattr->mode;
      pjentry->pattr->uid = reply_jentry.par_pattr->uid;
      pjentry->pattr->gid = reply_jentry.par_pattr->gid;
      pjentry->pattr->nlink = reply_jentry.par_pattr->nlink;
      pjentry->pattr->ctime = reply_jentry.par_pattr->ctime;
      pjentry->pattr->atime = reply_jentry.par_pattr->atime;
      pjentry->pattr->mtime = reply_jentry.par_pattr->mtime;
      pjentry->pattr->size = reply_jentry.par_pattr->size;

      pjentry->pattr->blocks = 1;
      pjentry->pattr->blksize = MAX(root->layout.stripe_unit, 4096);
    }
    // for rename if it has
    if (reply_jentry.par_pattr2) {
      pjentry->pattr2->parentid.inode = 0;
      pjentry->pattr2->fid.inode = reply_jentry.parentid2;
      pjentry->pattr2->fid.validator = reply_jentry.par_pattr2->ctime;
      // the parent changed to dest parent
      pjentry->attr->parentid.validator = reply_jentry.par_pattr2->ctime;
      len = reply_jentry.name2.length();
      pjentry->name2 = new char[len+1];
      reply_jentry.name2.copy(pjentry->name2, len, 0);
      pjentry->name2[len] = '\0';
      
      pjentry->pattr2->mode = reply_jentry.par_pattr2->mode;
      pjentry->pattr2->uid = reply_jentry.par_pattr2->uid;
      pjentry->pattr2->gid = reply_jentry.par_pattr2->gid;
      pjentry->pattr2->nlink = reply_jentry.par_pattr2->nlink;
      pjentry->pattr2->ctime = reply_jentry.par_pattr2->ctime;
      pjentry->pattr2->atime = reply_jentry.par_pattr2->atime;
      pjentry->pattr2->mtime = reply_jentry.par_pattr2->mtime;
      pjentry->pattr2->size = reply_jentry.par_pattr2->size;

      pjentry->pattr2->blocks = 1;
      pjentry->pattr2->blksize = MAX(root->layout.stripe_unit, 4096);
    }

    if (reply_jentry.pattr2 && (0 != reply_jentry.fid2.val)) {
      pjentry->attr2->parentid.inode = reply_jentry.parentid2;
      pjentry->attr2->parentid.validator = reply_jentry.par_pattr2->ctime;
      pjentry->attr2->fid.validator = reply_jentry.pattr2->ctime;
      pjentry->attr2->mode = reply_jentry.pattr2->mode;
      pjentry->attr2->uid = reply_jentry.pattr2->uid;
      pjentry->attr2->gid = reply_jentry.pattr2->gid;
      pjentry->attr2->nlink = reply_jentry.pattr2->nlink;
      pjentry->attr2->ctime = reply_jentry.pattr2->ctime;
      pjentry->attr2->atime = reply_jentry.pattr2->atime;
      pjentry->attr2->mtime = reply_jentry.pattr2->mtime;
      // if is dir, size equals rstat->rbytes
      // if is file, size equals inode->size
      pjentry->attr2->size = reply_jentry.pattr2->size;
      if (reply_jentry.pattr2->is_dir()) {
	// existing dest for rename should not be dir
	assert(0);
      }
      pjentry->attr2->blocks = (reply_jentry.pattr2->size + 511) >> 9;
      pjentry->attr2->blksize = MAX(root->layout.stripe_unit, 4096);
    }

    map< uint64_t, pair<__u8, mh_journal_entry*> >::reverse_iterator  prev =
      jentry_prefetch_buffer.rbegin();

    if (prev != jentry_prefetch_buffer.rend()) {
      if (prev->second.second->seq + prev->second.second->len != pjentry->seq)
	assert(0);
    }
    // 1 means just fetching back, not in mh processing yet
    pair<bool, mh_journal_entry*> jpair = make_pair(1, pjentry);
    // insert into the prefetch buffer
    journal_buffer_lock.Lock();

    pair< map<uint64_t, pair<__u8, mh_journal_entry*> >::iterator, bool > ret =
      jentry_prefetch_buffer.insert(make_pair(pjentry->seq, jpair));
    if (!ret.second) {
      ldout(cct, 1) << " prefetch jentrylist num " << i
		    << "jentry_prefetch_buffer found dup seq "
		    << pjentry->seq
		    <<" will free it's memery "
		    << dendl;
    ...
    ldout(cct, 20) << " prefetch jentrylist num " << i <<
      " op " << pjentry->op <<
      " fid " << hex << pjentry->attr->fid.inode <<
      " validator " << pjentry->attr->fid.validator <<
      " parentid " << hex << pjentry->attr->parentid.inode <<
      " mode " << pjentry->attr->mode <<
      " seq " << dec << pjentry->seq <<
      " len " << pjentry->len <<
      " blocks " << pjentry->attr->blocks <<
      " blksize " << pjentry->attr->blksize <<
      " name " << pjentry->name <<
      " parent name " << pjentry->name2  << dendl;

    if (mh_op_rename == pjentry->op)
      ldout(cct, 1) << "prefetch-rename " << i <<
	" op " << pjentry->op <<
	" srcfid " << hex << pjentry->attr->fid.inode <<
	" src_dname " << pjentry->name <<
	" srcpfid " << hex << pjentry->pattr->fid.inode <<
	" destpfid " << hex << pjentry->attr->parentid.inode <<
	" destp_dname " << pjentry->name2 << dendl;
  }
  // if the hold journal request is waiting for the prefetch reply
  // signal it
out:
  journal_buffer_lock.Lock();
  assert(prefetch_journal_outstanding == 1);
...
bool Client::_vxattrcb_uuid_exists(Inode *in)
{
  //becase this must exists after Client::path_walk
  return true;
}
size_t Client::_vxattrcb_uuid(Inode *in, char *val, size_t size)
{
  char fsid[36+1] = "";
  inodeno_t ino = _get_inodeno(in);
  char *tmp = fsid;
  cct->_conf->get_val("fsid", &tmp, 37);

  ldout(cct, 20) << "_vxattrcb_uuid  fsid: " << fsid 
		 << " ino: "<< ino << dendl;
  return snprintf(val, size, "%s:00000000-0000-0000-0000-%012lx", fsid, ino.val);
}
...
const Client::VXattr Client::_dir_vxattrs[] = {
  ...
  {
    name: "xtao.uuid",
    getxattr_cb: &Client::_vxattrcb_uuid,
    readonly: true,
    hidden: true,
    exists_cb: &Client::_vxattrcb_uuid_exists,
  },
