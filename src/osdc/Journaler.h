...
class C_MDS_RetryJentryRequest;
...
public:
  ...
  class Header {
    ...
    Header(const char *m="") :
      trimmed_pos(0), expire_pos(0), unused_field(0), write_pos(0), ad_release_pos(0),
      magic(m), stream_format(-1) {
    }
	...
	f->dump_unsigned("ad_release_pos", ad_release_pos);
  ...
  //Peng
  void jentry_buffer_insert(journal_entry* pjentry, uint64_t release_pos);
  void jentry_buffer_release(uint64_t seq);
  void update_release_pos(uint64_t seq, uint64_t len);

  Header last_committed;
  list<C_MDS_RetryJentryRequest*> jhold_wl;
...
private:
  ...
  bufferlist read_buf;
  //Peng ad reader
  uint64_t ad_read_pos;
  uint64_t ad_release_pos;
  uint64_t ad_release_disk_pos;
  uint64_t ad_requested_pos;
  uint64_t ad_received_pos;
  bufferlist ad_read_buf;
  ...
  //Peng
  uint64_t ad_fetch_len;
  uint64_t temp_ad_fetch_len;
  uint64_t ad_prefetch_outstanding;

  // for wait_for_readable()
  C_OnFinisher *on_readable;
  C_OnFinisher     *on_ad_readable;
  ...
  //Peng
  map<uint64_t, bufferlist> ad_prefetch_buf;
  map<uint64_t, journal_entry> jentry_buffer;
  void _finish_ad_read(int r, uint64_t offset, bufferlist &bl);
  void _issue_read(uint64_t len); // read some more
  void _issue_ad_read(uint64_t len);
  void _prefetch(); // maybe read ahead
  void _ad_prefetch();
  void _assimilate_ad_prefetch();
  class C_Read;
  friend class C_Read;
  class C_RetryRead;
  ...
  //Peng
  class C_AdRead;
  friend class C_AdRead;
  class C_RetryAdRead;
  friend class C_RetryAdRead;
...
public:
  Journaler(inodeno_t ino_, int64_t pool, const char *mag, Objecter *obj,
    ...
    ad_read_pos(0), ad_requested_pos(0), ad_received_pos(0),
    fetch_len(0), temp_fetch_len(0), ad_fetch_len(0), temp_ad_fetch_len(0),
    ad_prefetch_outstanding(0), on_readable(0), on_ad_readable(0), on_write_error(NULL),
    called_write_error(false),expire_pos(0), trimming_pos(0), trimmed_pos(0), ad_release_disk_pos(0),
    readable(false), write_iohint(0), stopping(false), metahunter(metahunter_)
  {
  }
  ...
  //Peng
  void wait_for_ad_readable(Context *onfinish);
  void insert_jhold_waitinglist(C_MDS_RetryJentryRequest *retryctx);
  ...
  bool init_ad_read_pos();
  void set_ad_pos(int64_t p) {
    lock_guard l(lock);
     ad_read_pos = ad_received_pos = ad_requested_pos = ad_release_pos
       = ad_release_disk_pos = p;
  }
  bool allow_mh();
...
public:
  ...
  //Peng
  bool is_ad_readable();
  uint64_t get_ad_read_pos() const { return ad_read_pos; }
  uint64_t get_ad_release_pos() const { return ad_release_pos; }

  //void add_to_release_pos(uint64_t len) { ad_release_pos += len; }
  void dec_ad_read_pos(uint64_t len);
  bool try_ad_read_entry(bufferlist& bl, uint64_t& len);
  void ad_force_flush();
