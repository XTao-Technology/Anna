...
//Peng
void EMetaBlob::fullbit::get_inode_value(ad_attr_t &inode_attrs,
					 map<string , bufferptr>& inode_xattrs)
{
  // do i need the deep copy ??? write a test program to verify
  //for (map<string, bufferptr>::iterator p = xattrs.begin();
  //     p != xattrs.end();
  //	++p) {
  ...
  inode_xattrs = xattrs;
#endif
  inode_attrs.mode = inode.mode;
  inode_attrs.nlink = inode.nlink;
  inode_attrs.ctime = inode.ctime;
  inode_attrs.mtime = inode.mtime;
  inode_attrs.atime = inode.atime;
  inode_attrs.uid = inode.uid;
  inode_attrs.gid = inode.gid;
  if (inode.is_dir()) {
    inode_attrs.size = inode.rstat.rbytes;
    inode_attrs.nsubdirs = inode.dirstat.nsubdirs;
    inode_attrs.nfiles = inode.dirstat.nfiles;
  } else {
    inode_attrs.size = inode.size;
    inode_attrs.nsubdirs = 0;
    inode_attrs.nfiles = 0;
  }
}
...
//Peng
void EMetaBlob::ad_restore_create(MDSRank *mds, journal_entry_t& jentry)
{
  ad_dump_lump(mds);
  //assert(logseg);
  // currently we only suppose the element was already in the mds cache
  // (that means the ad should read the journal entry after mds journal replay done)
  // we should latter think of the case that the corresponding element was
  // not in the mds cache, it means we should consider when we insert the element
  // into the cache, how do we deal with if the same element was already in the cache?
  
  // e.x /a/b/c, we get "dir b" first
  list<dirfrag_t>::reverse_iterator lp = lump_order.rbegin();
  dirlump &lump = lump_map[*lp];
  // lump.is_new() means "mkdir" op? can not be used for create file??
  //assert(lump.is_new());
  
  inodeno_t par_ino = (*lp).ino;
  string dname, par_dname;
  inodeno_t ino;
  map<string, bufferptr>inode_xattrs, par_inode_xattrs;
  ad_attr_t inode_attrs, par_inode_attrs;
  lump._decode_bits();
  uint32_t i = 0;
  for (list< ceph::shared_ptr<fullbit> >::const_iterator pp = lump.get_dfull().begin();
       pp != lump.get_dfull().end();
       ++pp) {
    // TODO: this assert will fail if there exists snapshot, refer to
    // journal_cow_dentry
    // if there exist snapshot, how about get the tail of the item from dfull??
    assert(0 == i);
    // should only have one dentry for "openc"
    ceph::shared_ptr<fullbit> p = *pp;
    dname = p->dn;
    ino = p->inode.ino;
    // get child ino's attr and xattr
    p->get_inode_value(inode_attrs, inode_xattrs);
    i++;
  }

  i = 0;
  if (!MDS_INO_IS_BASE(lp->ino)) {
    dirlump &par_par_lump = lump_map[*(++lp)];
    par_par_lump._decode_bits();
    for (list< ceph::shared_ptr<fullbit> >::const_iterator pp =
	   par_par_lump.get_dfull().begin();
	 pp != par_par_lump.get_dfull().end();
	 ++pp) {
      assert(0 == i);
      ceph::shared_ptr<fullbit> p = *pp;
      par_dname = p->dn;
      p->get_inode_value(par_inode_attrs, par_inode_xattrs);
      i++;
    }
  } else {
    for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin();
	 pp!= roots.end();
	 ++pp) {
      assert(0 == i);
      ceph::shared_ptr<fullbit> p = *pp;
  ...
  // now fully got what we need for create op
  dout(20) << "after create parentid " << par_ino.val << " name " << dname <<
    " parent name " << par_dname <<
    " ino " << ino << " ino.val " << ino.val << " mtime " << inode_attrs.mtime << 
    " parent mtime " << par_inode_attrs.mtime << " parent size "<<
    par_inode_attrs.size << dendl;

  jentry =  journal_entry(0, op_create, par_ino, ino, dname, par_dname,
			  &inode_attrs, &par_inode_attrs, inode_xattrs);
  return;
}

//Peng
void EMetaBlob::ad_restore_attrs(MDSRank *mds, journal_entry_t& jentry)
{
  dout(20) << "EMetaBlob.ad_restore_setxattr " << lump_map.size() <<
    " dirlumps by " << client_name << dendl;
  ad_dump_lump(mds);
  string dname;
  inodeno_t ino;
  inodeno_t par_ino;
  map<string, bufferptr> inode_xattrs;
  ad_attr_t inode_attrs;
  uint32_t i= 0;
  dirlump *lump = NULL;
  list<dirfrag_t>::reverse_iterator lp = lump_order.rbegin();
  bool is_stray = MDS_INO_IS_STRAY((*lp).ino);
  
  if (lump_order.empty()) {
    for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin(); pp != roots.end(); ++pp) {
      if (MDS_INO_ROOT == ((*pp)->inode.ino)) {
        dout(1) << " found setattr to root " << dendl;
        dname = "/";
        ino = (*pp)->inode.ino;
        (*pp)->get_inode_value(inode_attrs, inode_xattrs);
      }
    }
    goto out;
  }
...
out:
  dout(20) << "after setattr file name " << dname << "ino " << ino
          << " size  " << inode_attrs.size
          << " mode " << inode_attrs.mode
          << "nfiles " << inode_attrs.nfiles
          << "nsubdirs " << inode_attrs.nsubdirs
          << " ctime "<< inode_attrs.ctime
          << " atime " << inode_attrs.atime
          << " mtime " << inode_attrs.mtime
          << dendl;
  for (map<string, bufferptr>::iterator p = inode_xattrs.begin();
       p != inode_xattrs.end();
       ++p) {
    dout(20) << "xattr key " << p->first << " value " << p->second.c_str()
            << dendl;
  }

  if (!is_stray) {
    jentry =  journal_entry(0, op_setattr, par_ino, ino, dname, "",
			    &inode_attrs, NULL, inode_xattrs);
  } else {
    jentry =  journal_entry(0, op_none, par_ino, ino, dname, "",
			    &inode_attrs, NULL, inode_xattrs);
  }
...
//Peng
void EMetaBlob::ad_restore_mkdir(MDSRank *mds, journal_entry_t& jentry)
{
  dout(20)<< "EMetaBlob.ad_restore_mkdir " <<lump_map.size() <<
    " dirlumps by " << client_name<<dendl;
  ...
  list<dirfrag_t>::reverse_iterator lp = lump_order.rbegin();
  dirlump &lump = lump_map[*lp];
  inodeno_t ino = (*lp).ino;
  dirlump &par_lump = lump_map[*(++lp)];
  string dname;
  string par_dname;
  // since lump is for dir, mkdir op will mark_new to this dirlump
  // you should have a further thought about why it was added
  // into the LogSegment's new_dirfrags?
  assert(lump.is_new());
  // mkdir will modify the parent dir 's attr (time, nlinks ....), so we need to 
  // extract its parent info
  // e.x  "/a/b/c/"
  // lump is "dir c"
  // par_lump is "dir b"
  // par_par_lump is "dir a"
  inodeno_t par_ino = (*lp).ino;
  ad_attr_t inode_attrs, par_inode_attrs;
  map<string, bufferptr> inode_xattrs, par_inode_xattrs;
  par_lump._decode_bits();
  uint32_t i = 0;
  for (list< ceph::shared_ptr<fullbit> >::const_iterator pp =
	 par_lump.get_dfull().begin();
       pp != par_lump.get_dfull().end();
       ++pp) {
    // make sure the par lump only contains the new dir
    // TODO: this assert will fail if there exists snapshot, refer to
    // journal_cow_dentry
    // if there exist snapshot, how about get the tail of the item from dfull??
    assert(0 == i);
    ceph::shared_ptr<fullbit> p = *pp;
    assert(p->inode.is_dir());
    dname = p->dn;
    assert(ino == p->inode.ino);
    p->get_inode_value(inode_attrs, inode_xattrs);
    i++;
  }
  
  i = 0;
  // TODO: verify the deep copy of fullbit
  // list< ceph::shared_ptr<fullbit> > fullbit_list;
  // fullbit_list = roots or fullbit_list = par_par_lump.get_dfull()
  ...
  // print out for debug
  dout(20) << "after mkdir " << dendl;
  dout(20) << "new dir name " << dname << " ino " << ino << " size is " <<
    inode_attrs.size << " mode is: " << inode_attrs.mode
	  << " nfiles " << inode_attrs.nfiles
	  << " nsubdirs " << inode_attrs.nsubdirs << dendl;
  ...
  dout(20) << "parent dir name" << par_dname << " parrent ino " << par_ino <<
    " parent size " << par_inode_attrs.size <<
    " mode is: " << par_inode_attrs.mode <<
    " nfiles " << par_inode_attrs.nfiles <<
    " nsubdirs " << par_inode_attrs.nsubdirs << dendl;
  
  for (map<string, bufferptr>::iterator p = par_inode_xattrs.begin();
       p != par_inode_xattrs.end();
       ++p) {
    dout(20) << "parent xattr key " << p->first << " parent value "
	    << p->second.c_str() << dendl;
  }

  jentry =  journal_entry(0, op_mkdir, par_ino, ino, dname, par_dname,
			  &inode_attrs, &par_inode_attrs, inode_xattrs);
  return;
}


void EMetaBlob::ad_dump_lump(MDSRank *mds)
{
  dout(20) << "dump-lump" << dendl;
  string fullpath;
  uint32_t  i = 0;
  uint32_t j = 0;
  for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin(); pp != roots.end(); ++pp) {
    dout(20) << "roots fullbit " << i <<
      " dname " << (*pp)->dn <<
      " ino " << (*pp)->inode.ino <<
      " is_base " << MDS_INO_IS_BASE((*pp)->inode.ino) << dendl;
    i++;
  }
  ...
  for (list<dirfrag_t>::iterator lp= lump_order.begin();
       lp != lump_order.end();
       ++lp) {
    dout(20) << "*********lump order " << i << dendl;
    dout(20) << " dirfrag_t is " << (*lp) << dendl;

    if (MDS_INO_IS_MDSDIR((*lp).ino) ||
	MDS_INO_IS_STRAY((*lp).ino)) {
      continue;
    }
	...
	" dname " << (*pp)->dn <<
	" ino " << (*pp)->inode.ino <<
	" is base " << MDS_INO_IS_BASE((*pp)->inode.ino) << dendl;
      j++;
    }
    j = 0;
  ...
  dout(10) << "fullpath is " << fullpath << dendl;
}

void EMetaBlob::ad_dump_lump_rename(MDSRank *mds)
{
  dout(20) << "dump-lump" << dendl;
  uint32_t  i = 0;
  uint32_t j = 0;
  for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin(); pp != roots.end(); ++pp) {
    dout(1) << "roots fullbit " << i <<
      " dname " << (*pp)->dn <<
      " ino " << (*pp)->inode.ino <<
      " is_base " << MDS_INO_IS_BASE((*pp)->inode.ino) << dendl;
    i++;
  ...
  for (list<dirfrag_t>::iterator lp= lump_order.begin();
       lp != lump_order.end();
       ++lp) {
    dout(1) << "*********lump order " << i << dendl;
    dout(1) << " dirfrag_t is " << (*lp) << dendl;
    dirlump &lump = lump_map[*lp];
    lump._decode_bits();
    j = 0;
    for (list< ceph::shared_ptr<fullbit> >::const_iterator pp = lump.get_dfull().begin();
...
// for debug hardlink
void EMetaBlob::ad_dump_lump_link(MDSRank *mds)
{
  dout(1) << "dump-lump-link" << dendl;
  uint32_t  i = 0;
  uint32_t j = 0;
  for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin(); pp != roots.end(); ++pp) {
    dout(1) << "roots fullbit " << i <<
...
// for debug hardlink
void EMetaBlob::ad_dump_lump_unlink(MDSRank *mds)
{
  dout(1) << "dump-lump-unlink" << dendl;
  uint32_t  i = 0;
  uint32_t j = 0;
  for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin(); pp != roots.end(); ++pp) {
    dout(1) << "roots fullbit " << i <<
...
void EMetaBlob::ad_restore_unlink_local(MDSRank *mds, journal_entry_t &jentry)
{
  //ad_dump_lump(mds);
  ad_dump_lump_unlink(mds);

  inodeno_t ino, par_ino;
  string ino_name;
  ad_attr_t inode_attrs, par_inode_attrs;
  map<string, bufferptr> inode_xattrs, par_inode_xattrs;
  char s[20];

  dirlump *stray_lump = NULL;
  uint32_t idx = 0;
  bool isdir = false;
  // for debug
  bool found_child = 0;
  ...
  // this round locate nullbit and stray if it has
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin(); rlp != lump_order.rend(); ++rlp) {
    if (MDS_INO_IS_MDSDIR((*rlp).ino))
      continue;
  
    if (MDS_INO_IS_STRAY((*rlp).ino)) {
      assert(NULL == stray_lump);
      stray_lump = &lump_map[*rlp];
      continue;
    }

    dirlump &lump = lump_map[*rlp];
    lump._decode_bits();
    for (list<nullbit>::const_iterator p = lump.get_dnull().begin();
	p != lump.get_dnull().end();
      ++p) {
  ...
  // try to get null's parent's attrs
  for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin();
       pp != roots.end();
       ++pp) {
    // root is parent?
    ceph::shared_ptr<fullbit> p = *pp;
    if (p->inode.ino == par_ino) {
      par_dname = "/";
      p->get_inode_value(par_inode_attrs, par_inode_xattrs);
      found_parent = true;
      break;
...
child:
  assert(found_parent);
  // hardlink will not have the stray lump
  if (stray_lump) {
    stray_lump->_decode_bits();
    idx = 0;
    for (list< ceph::shared_ptr<fullbit> >::const_iterator pp = stray_lump->get_dfull().begin();
	 pp != stray_lump->get_dfull().end();
	 ++pp) {
      // get the unlink ino from straydir 
      assert(0 == idx);
      ceph::shared_ptr<fullbit> p = *pp;
      ino = p->inode.ino;
      snprintf(s, sizeof(s), "%llx", (unsigned long long)(ino.val));
      ino_name = s;
      // since the name of unlink entry in stray lump fullbit list equals ino
      assert (p->dn == ino_name);
      p->get_inode_value(inode_attrs, inode_xattrs);
      found_child = true;
      if (p->inode.is_dir())
	isdir = true;
  ...
  // it is hardlink remove, please note we can not hardlink to a dir
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin(); rlp != lump_order.rend(); ++rlp) {
    dirlump &lump = lump_map[*rlp];
    lump._decode_bits();
    for (list< ceph::shared_ptr<fullbit> >::const_iterator pp = lump.get_dfull().begin();
...
out:
  assert(found_child && found_parent);
  // print the outcome for debug
   
  dout(1) << "after unlink, have straylump " << (stray_lump ? 1: 0 )<<dendl;
  dout(1) << "unlink name " << dname << " ino " << ino << " size is " <<
    inode_attrs.size << " nlink is " << inode_attrs.nlink
	  <<" mode is: " << inode_attrs.mode
	  << " nfiles " << inode_attrs.nfiles
	  << " nsubdirs " << inode_attrs.nsubdirs << dendl;
  ...
  for (map<string, bufferptr>::iterator p = inode_xattrs.begin();
       p != inode_xattrs.end();
       ++p) {
    dout(1) << "xattr key " << p->first << " value " << p->second.c_str()
	    << dendl;
  }
  
  dout(1) << "after unlink hardlink parent dir name " << par_dname <<
    " parrent ino " << par_ino <<
    " parent size " << par_inode_attrs.size <<
    " mode is: " << par_inode_attrs.mode <<
    " nfiles " << par_inode_attrs.nfiles <<
  ...
  for (map<string, bufferptr>::iterator p = par_inode_xattrs.begin();
       p != par_inode_xattrs.end();
       ++p) {
    dout(1) << "parent xattr key " << p->first << " parent value "
	    << p->second.c_str() << dendl;
  }
  
  
  jentry =  journal_entry(0, (isdir ? op_rmdir : op_unlink),
			  par_ino, ino, dname, par_dname,
			  &inode_attrs, &par_inode_attrs,
			  inode_xattrs);
  return;
}
...
/*
* restore rename highly depend on the op sequence during _rename_prepare
* and currently only support single active mds scenario.
* 
* case 1:
* take the example "mv /bar/src /foo/dest", suppose dest already existed
* the op sequence in _rename_prepare() should be as follows:
*
* 1. add_dir_context("straydir"),  add_dir("straydir")
*     lump_order  => "mdsdir", "straydir"
*     lump_map['mdsdir'].dfull  => 'straydir'
*
* 2. predirty_journal_parents(.., "dest" oldin, "foo" dir, -1)
*      lump_order => "mdsdir", "straydir",  "/" ,
*      lump_map['/'].dfull => "foo"
* 
* 3.  predirty_journal_parents(.., "dest" oldin, "straydir", +1)
*      lump_map['mdsdir'].dfull => 'straydir' , 'straydir'
*
* 4. predirty_journal_parents(.., "src" srcin, "bar" dir, -1 )
*     lump_order => "mdsdir", "straydir",  "\", 
*     lump_map['/'].dfull => 'foo',  'bar'
*
* 5. predirty_journal_parents(.., "src" srcin, dest "foo" dir, +1))
*     lump_map['/'].dfull => 'foo', 'bar', 'foo'
* 
* 6. add_primary_dentry(straydn "dest", "dest" oldin, ...)
*     lump_map['straydir'].dfull => 'dest'
*
* 7. add_primary_dentry("dest" dn, "src" srci, ...)
*     lump_order => "mdsdir", "straydir", "/", "foo"
*     lump_map['foo'].dfull => "dest"
*
* 8. add_null_dentry("src" dn, ..)
*      lump_order => "mdsdir", "straydir", "/", "foo", "bar"
*      lump_map['bar'].dnull => "src"
*
*
*  case 2:
* take mv /bar/src /foo/dest   (it still holds true when
* src and dest is dir)
* if the dest does not exist, the op sequence should be as:
* 
* 1. predirty_journal_parents(..., "src" srci,  "bar" dir, -1)
*      lump_order => '/' 
*      lump_map['/'].dfull => 'bar'
*
* 2. predirty_journal_parents(..., "src" srci, "foo" dir, +1)
*     lump_order => '\' 
*     lump_map['/'].dfull => 'bar', 'foo'
*
* 3. add_primary_dentry("dest" dn, "src" srci, true, true)
*     lump_order => '/', 'foo'
*     lump_map['foo'].dfull => "dest"
*
* 4. add_null_dentry("src" dn, true)
*      lump_order => '/', 'foo', 'bar'
*      lump_map['bar'].dnull => "src"
*/
...
void EMetaBlob::ad_restore_rename(MDSRank *mds, journal_entry_t &jentry)
{
  ad_dump_lump_rename(mds);
  
  inodeno_t srcino, srcpino, destino, destpino;
  inodeno_t src_ancestor_ino;
  inodeno_t remote_ancestor_ino;
  inodeno_t remote_pino;
  ad_attr_t srci_attrs, srcpi_attrs, desti_attrs, destpi_attrs;
  map<string, bufferptr> srci_xattrs, srcpi_xattrs, desti_xattrs, destpi_xattrs;
  uint32_t idx = 0;
  bool  found_srcp = 0;
  bool found_destp = 0;
  bool found_remote = 0;
  ...
  dirlump *stray_lump = NULL;
  string src_dname, srcp_dname, dest_dname, destp_dname = "";
  // put all src ancestors in the set
  set<uint64_t> src_ancestors;
  set<uint64_t> remote_ancestors;

  
  // find remote and its ancestor first
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
       rlp != lump_order.rend(); ++rlp) {
    dirlump &lump = lump_map[*rlp]; 
    lump._decode_bits();
    idx = 0;
    for (list<remotebit>::const_iterator p = lump.get_dremote().begin();
	 ...
	 ++p) {
      assert(0 == idx);
      srcino = destino = p->ino;  // seems mh does not need this?
      dest_dname = p->dn;
      destpino = remote_pino = remote_ancestor_ino = (*rlp).ino;
      remote_ancestors.insert(remote_ancestor_ino.val);
      found_remote = true;
      found_destp = true;
      idx++;
    }
  ...
  if (found_remote) {
    ...
    for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
	 rlp != lump_order.rend(); ++rlp) {
	  ...
	  ++pp) {
	ceph::shared_ptr<fullbit> p = *pp;
	if (p->inode.ino == srcino) {
	  p->get_inode_value(srci_attrs, srci_xattrs);
	}
	if (p->inode.ino == remote_pino) {
	  destp_dname = p->dn;
	  p->get_inode_value(destpi_attrs, destpi_xattrs);
	}
	if (p->inode.ino == remote_ancestor_ino) {
	  remote_ancestor_ino = (*rlp).ino;
	  remote_ancestors.insert(remote_ancestor_ino.val);
	}
      }
    }
  ...
  // locate stray first if it has
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
       rlp != lump_order.rend(); ++rlp) {
    // locate straydir first
    if (MDS_INO_IS_STRAY((*rlp).ino)) {
      stray_lump = &lump_map[*rlp];
      stray_lump->_decode_bits();
      idx = 0;
	...
	assert(0 == idx);
	ceph::shared_ptr<fullbit> p = *pp;
	destino = p->inode.ino;  // seems mh does not need this?
	p->get_inode_value(desti_attrs, desti_xattrs);  // mh does not need this?
      }
    }
  }
  
  // please be aware of handling these cases:
  // case 1:
  // mv  /a/b/c/d/bar/src   /foo/dest  
  // ("dest" exits and  the "src" and "dest" are all ordinary files)
  // finally, the lump_order might look like this:
  // lump_order => "mdsdir", "straydir", "/", "a", "b", "c", "d", "foo", "bar"
  //
  //case 2:
  //mv /a/b/c/d/bar/src/   /e/f/g/foo/dest
  // ("dest" exits and the "src" and "dest" are all ordinary files)
  //
  // case 3:
  // mv /a/b/c//bar/src /d/e/f/foo/dest
  // "dest" not exits, 
  // lump_order => "/", "a", "b", "c", "d", "e", "f", "foo", "bar"
  ...
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
    ...
    // hardlink related background rename will in straydir's dnull
    //if (MDS_INO_IS_STRAY((*rlp).ino))
    //  continue;
    ...
    for (list<nullbit>::const_iterator p = lump.get_dnull().begin();
	 ...
	 ++p) {
      assert(0 == idx);
      src_ancestor_ino = srcpino = (*rlp).ino;
      src_ancestors.insert(src_ancestor_ino.val);
      dout(1) << "from dnull src_ancestors insert ino " << src_ancestor_ino << dendl;
      src_dname = p->dn;
      found_srcp = 1;
    }
    if (found_srcp)
  ...
  assert(found_srcp);
  if (MDS_INO_IS_STRAY(srcpino)) {
    // this is the case that hardlink point to a file, whose parent has been
    // deleted, if we visit this hardlink file, it will trigger a background rename op
    // we will filter this out
    jentry.op = op_none;
    return;
  }
  
  // in this round we will locate all the src ancestors
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
       rlp != lump_order.rend(); ++rlp) {
    if (MDS_INO_IS_MDSDIR((*rlp).ino))
      continue;
    ...
    dirlump &lump = lump_map[*rlp];
    lump._decode_bits();
    for (list< ceph::shared_ptr<fullbit> >::const_iterator it = lump.get_dfull().begin();
	 it != lump.get_dfull().end(); ++it) {
      ceph::shared_ptr<fullbit> itp = *it;
      if (src_ancestor_ino == itp->inode.ino) {
	src_ancestor_ino = (*rlp).ino;
	src_ancestors.insert(src_ancestor_ino.val);
	dout(1) << "from dfull src_ancestors insert ino " << src_ancestor_ino << dendl;
      }
    }
  }

  // in this round, we extract the src ancestors, the left over is the dest
  // related
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
       rlp != lump_order.rend(); ++rlp) {
    if (MDS_INO_IS_MDSDIR((*rlp).ino))
    ...
    for (list< ceph::shared_ptr<fullbit> >::const_iterator it = lump.get_dfull().begin();
	 it != lump.get_dfull().end(); ++it) {
      ceph::shared_ptr<fullbit> itp = *it;
      if ((src_ancestors.find(itp->inode.ino.val) == src_ancestors.end()) &&
	  !found_remote) {
	// in the reverse order, the first non src_ancestors fullbit is the dest  
	destpino = (*rlp).ino;
	srcino = itp->inode.ino;
	dest_dname =  itp->dn;
	itp->get_inode_value(srci_attrs, srci_xattrs);
	found_destp = 1;
	break;
      }
    }
    if (found_destp) break;
  }
  assert(found_destp);
  ...
  for (list< ceph::shared_ptr<fullbit> >::iterator pp = roots.begin(); pp != roots.end(); ++pp) {
    ...
    if (p->inode.ino == srcpino) {
      srcp_dname = "/";
      p->get_inode_value(srcpi_attrs, srcpi_xattrs);
    }
    
    if (p->inode.ino == destpino) {
      destp_dname = "/";
      p->get_inode_value(destpi_attrs, destpi_xattrs);
    }
  }
  
  if ((MDS_INO_ROOT == srcpino) &&
      (MDS_INO_ROOT == destpino)) {
    goto result; 
  }
  ...
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
    ...
    for (list< ceph::shared_ptr<fullbit> >::const_iterator it= lump.get_dfull().begin();
	 it != lump.get_dfull().end();
	 ++it) {
      ceph::shared_ptr<fullbit> itp = *it;
      if (!MDS_INO_IS_BASE(srcpino) && (itp->inode.ino == srcpino)) {
	srcp_dname = itp->dn;
	itp->get_inode_value(srcpi_attrs, srcpi_xattrs);
      }
      if (!MDS_INO_IS_BASE(destpino) && (itp->inode.ino == destpino)) {
	destp_dname = itp->dn;
	itp->get_inode_value(destpi_attrs, destpi_xattrs);
      }
    }
  }
result:
  // print the outcome for debug
  dout(1) << "after rename " << dendl;
  dout(1) << "rename  src name " << src_dname << " src ino " << srcino << " src parent name " <<
    srcp_dname << " src parent ino: " << srcpino
	  << " dest name " << dest_dname << " dest ino " << destino << " dest parent name "
	  << destp_dname << " dest parent ino "<< destpino << dendl;
  
  jentry =  journal_entry(0, op_rename,
			  srcpino, srcino, src_dname, srcp_dname,
			  &srci_attrs, &srcpi_attrs,
			  srci_xattrs,
			  destpino, destino, dest_dname, destp_dname,
			  &desti_attrs, &destpi_attrs);
...
//Peng hardlink
void EMetaBlob::ad_restore_link(MDSRank *mds, journal_entry_t &jentry)
{
  ad_dump_lump_link(mds);
  dirlump *lump = NULL;
  inodeno_t ino, par_ino;
  string dname, par_dname;
  bool found_remote = false;
  bool got_attr = false;
  bool got_pattr = false;
  ad_attr_t inode_attrs, par_inode_attrs; 
  map<string, bufferptr> inode_xattrs, par_inode_xattrs;
  uint32_t idx = 0;
  // first locate the remotebits
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
       rlp != lump_order.rend(); ++rlp) {
    // locate straydir first
    lump = &lump_map[*rlp];
  ...
  // now we got the ino and parrent ino, now try to get their attributes
  for (list<dirfrag_t>::reverse_iterator rlp = lump_order.rbegin();
       rlp != lump_order.rend(); ++rlp) {
    lump = &lump_map[*rlp];
    lump->_decode_bits();
...
out:
  assert(got_attr && got_pattr);
  // now fully got what we need for create op
  dout(1) << "after hardlink   parentid " << par_ino << " name " << dname <<
    " parent name " << par_dname <<
    " ino " << ino << " nlink " << inode_attrs.nlink << " mtime " << inode_attrs.mtime << 
    " size " << inode_attrs.size <<
    " parent mtime " << par_inode_attrs.mtime << " parent size "<<
    par_inode_attrs.size << dendl;
  
  // Peng think metahunter should support the op_link, 
  jentry =  journal_entry(0, op_create, par_ino, ino, dname, par_dname,
			  &inode_attrs, &par_inode_attrs, inode_xattrs);
  return;
...
// peng continue unlink with hardlink 
...
//Peng
void EUpdate::ad_replay(MDSRank *mds, struct journal_entry& jentry)
{
  if(("openc" == type) || ("symlink" == type) || ("mknod" == type)) {
    metablob.ad_restore_create(mds, jentry);
  } else if (("removexattr" == type) || ("setxattr" == type) || ("setattr" == type) || "cap update" == type) {
    metablob.ad_restore_attrs(mds, jentry);
  } else if ("mkdir" == type) {
    metablob.ad_restore_mkdir(mds, jentry);
  } else if ("unlink_local" == type) {
    // which include file and dir
    metablob.ad_restore_unlink_local(mds, jentry);
  } else if ("rename" == type) {
    metablob.ad_restore_rename(mds, jentry);
    //jentry.op = op_none;
  } else if ("link_local" == type) {
    metablob.ad_restore_link(mds, jentry);
    //jentry.op = op_none;
  }
  else {
    // the update type might be
    // "scatter_writebehind", "purge_stray".....
    jentry.op = op_none;
  }
}
