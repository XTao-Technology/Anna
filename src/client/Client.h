...
class Client : public Dispatcher, public md_config_obs_t {
 public:
  ...
  //yunfei
  string mds_name;
  uint32_t release_in_process;
  uint64_t prefetch_journal_outstanding;
  ...
  //Peng
  Finisher async_prefetch_journal;
...
protected:
  ...
  // wait for the prefetching journal
  list<Cond*> waiting_for_prefetch_journal;
  //wait for clearing jentry_prefetch_buffer
  list<Cond*> waiting_for_buffer_clean;
  FSMap *fsmap;
...
protected:
  ...
  //Peng
  //map< uint64_t, pair<bool, mh_journal_entry*> > mhjentry_buffer;
  map< uint64_t, pair<__u8, mh_journal_entry*> > jentry_prefetch_buffer;
  ...
  Mutex                  client_lock;
  // only used for protect the prefetch journal buffer 
  Mutex                  journal_buffer_lock;

  // helpers
  void wake_inode_waiters(MetaSession *s);
  void wait_on_list(list<Cond*>& ls);
  void wait_on_journal_buffer(list<Cond*>& ls);
  ...
  void dump_journal_entry(Formatter *f); // debug
...
private:
  ...
  bool _vxattrcb_uuid_exists(Inode *in);
  size_t _vxattrcb_uuid(Inode *in, char *val, size_t size);
...
public:
  ...
  //Peng
  int mh_open_journal(const char *mds);
  int ll_hold_journal();
  int mh_hold_journal(void* buffer, int buflen);
  xt_map_t* xt_map_encode(std::map<std::string, bufferptr>& map);
  int prefetch_journallist();
  void retry_prefetch_journal();
  int mh_get_root(void * buffer, int buflen);
  int mh_release_journal(uint64_t seq);
  void clear_jentry_prefetch_buffer();
...
//Peng used for prefetch journal entries
class C_Client_PrefetchJentryList : public Context {
private:
  Client *client;
public:
  C_Client_PrefetchJentryList(Client *c) : client(c) {}
  void finish(int r) {
    int res = client->prefetch_journallist();
    if (res < 0) {
      client->retry_prefetch_journal();
    }
  }
};
#endif
