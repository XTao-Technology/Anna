...
struct RGWZoneGroupPlacementTarget {
  ...
  bool user_permitted(list<string>& user_tags) {
    // If placement_targets's tags list is empty, it not ready yet.
    // User is not allowed to use it now.
...
public:
  RGWRados() : max_req_id(0), lock("rados_timer_lock"), watchers_lock("watchers_lock"), timer(NULL),
    ...
    /* initialize random seed for random_sched() */
    srand(time(NULL));
  }
  ...
  std::string random_sched(map<string, RGWZoneGroupPlacementTarget> placement_targets);
  string select_placement(list<string>& user_tags, map<string, RGWZoneGroupPlacementTarget> placement_targets);
