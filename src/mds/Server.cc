...
void Server::handle_client_session(MClientSession *m)
{
  ...
  case CEPH_SESSION_REQUEST_CLOSE:
	...
	assert(0);
      ...
      //Peng debug
      dout(1) << "peng handle client session close req" << dendl;
...
void Server::terminate_sessions()
{
  ...
  for (set<Session*>::const_iterator p = sessions.begin();
	...
	session->is_closed())
      continue;
    //Peng debug
    dout(1) << "peng terminate_sessions " << dendl;
    journal_close_session(session, Session::STATE_CLOSING, NULL);
  }
...
void Server::find_idle_sessions()
{
  ...
  while (1) {
    ...
    //Peng debug
    dout(1) << "peng find_idle_sessions will kill_session " << dendl;
...
void Server::kill_session(Session *session, Context *on_safe)
{
  if ((session->is_opening() ||
    ...
    //Peng debug
    dout(1) << "peng kill_session " << dendl;
...
void Server::journal_close_session(Session *session, int state, Context *on_safe)
{
  ...
  //Peng
  dout(1) << "peng journal_close_session " << dendl;
  ...
  while (!p.end()) {
    int op = (*p)->client_request->get_op(); 
...
void Server::handle_client_reconnect(MClientReconnect *m)
{
  ...
  //yunfei reset the last_cap_renew as the current time when send reconnect is case of 
  //being killed by find_idle_sessions function
...
//Peng
void Server::reply_ad_request(MDRequestRef& mdr, MClientReply *reply)
{
  assert(mdr.get());
  MClientRequest *req = mdr->client_request;
  
  dout(10) << "reply_ad_request " << reply->get_result() 
	   << " (" << cpp_strerror(reply->get_result())
	   << ") " << *req << dendl;
...
{
  dout(20) << "handle_client_request " << *req << dendl;
				  ...
				  file_layout_t *layout)
{
  ...
  if (mdr->session->info.prealloc_inos.size()) {
    ...
    dout(10) << "ivan prepare_new_inode alloc " << mdr->alloc_ino
	    << " inode " << *in
	    << dendl;
  }
...
void Server::handle_client_getattr(MDRequestRef& mdr, bool is_lookup)
{
  ...
  // reply  
  dout(10) << "reply to stat on " << *req << " ref: " << *ref <<dendl;
  mdr->tracei = ref;
  if (is_lookup)
...
void Server::handle_client_rename(MDRequestRef& mdr)
{
  ...
  if (!destdnl->is_null()) {
    ...
    dout(1) << "pengrename dest dn exists " << *destdn << dendl;
  ...
  if (linkmerge)
    //peng temp out dout(10) << " this is a link merge" << dendl;
    dout(1) << "pengrename this is a link merge" << dendl;

  // -- create stray dentry? --
  CDentry *straydn = NULL;
  if (destdnl->is_primary() && !linkmerge) {
    straydn = prepare_stray_dentry(mdr, destdnl->get_inode());
    //peng temp comment out dout(10) << " straydn is " << *straydn << dendl;
    dout(1) << "pengrename straydn is " << *straydn << dendl;
  } else if (mdr->straydn) {
    mdr->unpin(mdr->straydn);
    mdr->straydn = NULL;
  }
			     ...
			     CDentry *srcdn, CDentry *destdn, CDentry *straydn)
{
  ...
  if (srci->is_dir() && (destdn->is_auth() || force_journal_dest)) {
    //peng temp out dout(10) << " noting renamed dir ino " << srci->ino() << " in metablob" << dendl;
    dout(1) << " pengrename noting renamed dir ino " << srci->ino() << " in metablob" << dendl;
    metablob->renamed_dirino = srci->ino();
  } else if (oldin && oldin->is_dir() && force_journal_stray) {
    //peng temp out dout(10) << " noting rename target dir " << oldin->ino() << " in metablob" << dendl;
    dout(1) << " pengrename noting rename target dir " << oldin->ino() << " in metablob" << dendl;
  ...
  if (destdnl->is_primary()) {
    assert(straydn);
    if (straydn->is_auth()) {
      dout(1)<< "pengrename rename_prepare will add_dir_context(straydn->get_dir())" << dendl;
      dout(1) << "pengrename straydn is " << *straydn << dendl;
      /* peng think if the destdn doesnt exist before the rename,
       * the straydir will not be added into lump_order */
