...
typedef struct ad_attr{
  ...
  uint64_t nfiles;
  uint64_t nsubdirs;
  ad_attr() : mode(0), uid(0), gid(0), nlink(0),
		ctime(utime_t()),
		mtime(utime_t()),
		atime(utime_t()),
		size(0),
...
typedef struct journal_entry {
  ...
  string par_name2;
  ad_attr_t* pattr2;
  ad_attr_t* par_pattr2;

  void set_seq(uint64_t release_pos) {
    seq = release_pos;
  }
  void swap(journal_entry& jentry);

  journal_entry() : seq(0), op(op_none), parentid(0), fid(0),
		    pattr(NULL), par_pattr(NULL),
		    parentid2(0), fid2(0), pattr2(NULL), par_pattr2(NULL) { }

  journal_entry(int seq_num, ad_op_type_t op_type, inodeno_t par_ino,
		inodeno_t ino, string dname, string par_dname,
		ad_attr_t* attr, ad_attr_t* par_attr, map<string, bufferptr> xattr,
		inodeno_t par_ino2 = 0, inodeno_t ino2 = 0, string dname2 = "",
		string par_dname2 = "", ad_attr_t* attr2 = NULL,
		ad_attr_t* par_attr2 = NULL);

  journal_entry(const journal_entry& jentry);
  journal_entry& operator=(const journal_entry& jentry);

  ~journal_entry();
  //int journal_entry::total_seq = 0;
