...
// the new general journal entry and attr
/*
...
//Peng
  int ceph_mh_open_journal(struct ceph_mount_info *cmount,
			   const char *mds);
  int ceph_ll_hold_journal(struct ceph_mount_info *cmount);
  int ceph_mh_hold_journal_entry(struct ceph_mount_info *cmount,
				 void* buffer, int buflen);
  int ceph_mh_hold_journal_entrylist(struct ceph_mount_info *cmount,
				 void* buffer, int buflen);
  int ceph_mh_get_root(struct ceph_mount_info *cmount,
		       void* buffer, int buflen);
  int ceph_mh_release_journal_entry(struct ceph_mount_info *cmount,
				 uint64_t seq);
  int ceph_mh_shutdown(struct ceph_mount_info *cmount);
