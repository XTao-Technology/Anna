...
//Peng
extern "C" int ceph_mh_open_journal(class ceph_mount_info *cmount,
				    const char *mds)
{
  return (cmount->get_client())->mh_open_journal(mds);
}

extern "C" int ceph_ll_hold_journal(class ceph_mount_info *cmount)
{
  return (cmount->get_client())->ll_hold_journal();
}

extern "C" int ceph_mh_hold_journal_entry(class ceph_mount_info *cmount,
					  void* buffer, int buflen)
{
  return (cmount->get_client())->mh_hold_journal(buffer, buflen);
}


extern "C" int ceph_mh_get_root(class ceph_mount_info *cmount,
				void* buffer, int buflen)
{
  return (cmount->get_client())->mh_get_root(buffer, buflen);
}

extern "C" int ceph_mh_release_journal_entry(class ceph_mount_info *cmount,
				       uint64_t seq)
{
  //struct journal_entry jentry;
  return (cmount->get_client())->mh_release_journal(seq);
  //*ppjentry = new mh_journal_entry();
  // convert journal_entry to mh_journal_entry
  
}

extern "C" int ceph_mh_shutdown(class ceph_mount_info *cmount)
{
  (cmount->get_client())->shutdown();
  return 0;
}
/**
