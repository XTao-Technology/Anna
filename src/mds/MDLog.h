...
public:
  ...
  void ad_read_journal_entry_leak(journal_entry_t &jentry, MDRequestRef& mdr,
    Metahunter* metahunter);
  void ad_release_journal_entry(uint64_t seq, uint64_t len);
  uint64_t get_release_pos();
  bool ad_journal_init();
  bool allow_mh();
  void ad_read_journal_entry(journal_entry_t &jentry, MDRequestRef& mdr,
    Metahunter* metahunter);
  bool is_last_session_jentry_retry(MDRequestRef& mdr);
  void jentry_queue_wait(MDRequestRef& mdr, Metahunter *metahunter_);
...
public:
  ...
  void set_metahunter(Metahunter *metahunter_);
};
